...
for hoja, desc in [
    ("ESP","Estado de Situación Patrimonial"),("E.R.","Estado de Resultados"),
    ("EEPN","Estado de Evolución del Patrimonio Neto"),
    ("EF. IND.","Estado de Flujo de Efectivo (Método Indirecto)"),
    ("B. Uso","Bienes de Uso"),("Costo","Costo de Mercaderías Vendidas"),
    ("Gastos","Información Art. 64 Inc. b) — Ley 19.550 (Gastos por función)"),
...
# ═══════════════════════════════════════════════════════════════════════════
# ESP — lado a lado (RT9): ACTIVO izquierda | PASIVO derecha
# 9 columnas: A=ACTIVO label, B=nota ref, C=val25, D=val24rx, E=spacer,
#             F=PASIVO label, G=nota ref, H=val25, I=val24rx
# ═══════════════════════════════════════════════════════════════════════════
ws = wb.create_sheet("ESP")
NC_ESP = 9
set_cols(ws, [26, 11, 14, 14, 2, 26, 11, 14, 14])

r = 1
ws.merge_cells(start_row=r, start_column=1, end_row=r, end_column=NC_ESP)
c = ws.cell(r, 1); c.value = EMPRESA
c.font = Font(bold=True, size=12, name="Arial", color=C_HDR)
c.alignment = Alignment(horizontal="center"); r += 1

ws.merge_cells(start_row=r, start_column=1, end_row=r, end_column=NC_ESP)
c = ws.cell(r, 1); c.value = "ESTADO DE SITUACIÓN PATRIMONIAL"
c.font = Font(bold=True, size=11, name="Arial", color=C_HDR)
c.alignment = Alignment(horizontal="center"); r += 1

ws.merge_cells(start_row=r, start_column=1, end_row=r, end_column=NC_ESP)
c = ws.cell(r, 1)
c.value = f"Por el ejercicio N°{NRO_EJ} finalizado el {EJ25} | Comparativo al {EJ24} — (Expresado en moneda homogénea - Pesos - a la fecha de cierre)"
c.font = Font(italic=True, size=9, name="Arial")
c.alignment = Alignment(horizontal="center", wrap_text=True)
row_height(ws, r, 22); r += 1

# Cabeceras de columnas de valor
for ci, lbl in [(3, EJ25), (4, f"{EJ24}\n(reexp.)"), (8, EJ25), (9, f"{EJ24}\n(reexp.)")]:
    c = ws.cell(r, ci); c.value = lbl
    c.fill = fl(C_HDR); c.font = fw(bold=True, color="FFFFFF", size=9)
    c.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
row_height(ws, r, 28); r += 1

# Títulos de sección ACTIVO / PASIVO
...
def _esp(ws, r,
         al, an, av1, av2,   # ACTIVO: label, nota, val25, val24rx
         pl, pn, pv1, pv2,   # PASIVO: label, nota, val25, val24rx
         a_bg=None, p_bg=None, a_bold=False, p_bold=False):
    def _cell(ci, val, bold, bg, num=False, nota=False):
        c = ws.cell(r, ci)
        if val is not None: c.value = val
        c.font = Font(bold=bold, name="Arial", size=9,
                      color=C_HDR if bg in (C_TOT, C_HDR) else "000000")
        if bg: c.fill = fl(bg)
        if num and isinstance(val, (int, float)): c.number_format = NF
        if nota: c.alignment = Alignment(horizontal="left", vertical="center")
    _cell(1, al, a_bold, a_bg)
    _cell(2, an, False,  a_bg, nota=True)
    _cell(3, av1, a_bold, a_bg, num=True)
    _cell(4, av2, a_bold, a_bg, num=True)
    ws.cell(r, 5).fill = fl("FFFFFF")
    _cell(6, pl, p_bold, p_bg)
    _cell(7, pn, False,  p_bg, nota=True)
    _cell(8, pv1, p_bold, p_bg, num=True)
    _cell(9, pv2, p_bold, p_bg, num=True)

# 20 filas de cuerpo (ACTIVO izq | PASIVO der)
_esp(ws,r, "Activo corriente",None,None,None,    "Pasivo corriente",None,None,None,    a_bg=C_SEC,p_bg=C_SEC,a_bold=True,p_bold=True); r+=1
_esp(ws,r, "Caja y bancos","(Nota 2.1)",caja25,caja24, "Deudas:",None,None,None); r+=1
_esp(ws,r, "Inversiones",None,inv25 or None,None, "  Comerciales","(Nota 2.5.1)",dc25,dc24); r+=1
_esp(ws,r, "Créditos por ventas","(Nota 2.2)",cv25,cv24, "  Préstamos",None,None,None); r+=1
_esp(ws,r, "Otros créditos","(Nota 2.3)",oc25,oc24, "  Remuneraciones y cargas sociales","(Nota 2.5.3)",rem25,rem24); r+=1
_esp(ws,r, "Bienes de cambio","(Nota 2.4)",bc25,bc24, "  Cargas fiscales","(Nota 2.5.2)",df25,df24); r+=1
_esp(ws,r, "Otros activos",None,None,None, "  Deudas Sociales","(Nota 2.5.4)",ds25,ds24); r+=1
_esp(ws,r, "Total del activo corriente",None,ta25,ta24, "  Dividendos a pagar",None,None,None, a_bg=C_SUB,a_bold=True); r+=1
_esp(ws,r, "",None,None,None, "  Otras deudas",None,None,None); r+=1
_esp(ws,r, "Activo no corriente",None,None,None, "Total deudas",None,tp25,tp24, a_bg=C_SEC,p_bg=C_SUB,a_bold=True,p_bold=True); r+=1
_esp(ws,r, "Créditos por ventas",None,None,None, "Previsiones",None,None,None); r+=1
_esp(ws,r, "Otros créditos",None,None,None, "Total del pasivo corriente",None,tp25,tp24, p_bg=C_SUB,p_bold=True); r+=1
_esp(ws,r, "Bienes de cambio",None,None,None, "Pasivo no corriente",None,None,None, p_bg=C_SEC,p_bold=True); r+=1
_esp(ws,r, "Bienes de uso","(Anexo I)",bu25,None, "Deudas:",None,None,None); r+=1
_esp(ws,r, "Participaciones permanentes en Sociedades",None,None,None, "  Sociales","(Nota 2.7)",None,None); r+=1
_esp(ws,r, "Otras inversiones",None,None,None, "Previsiones",None,None,None); r+=1
_esp(ws,r, "Activos intangibles",None,None,None, "Total del pasivo no corriente",None,0.0,0.0, p_bg=C_SUB,p_bold=True); r+=1
_esp(ws,r, "Otros activos",None,None,None, " TOTAL DEL PASIVO",None,tp25,tp24, p_bg=C_HDR,p_bold=True); r+=1
_esp(ws,r, "Total del activo no corriente",None,bu25,None, " PATRIMONIO NETO  (según el EEPN)",None,pn25,pn24_rx, a_bg=C_SUB,p_bg=C_SUB,a_bold=True,p_bold=True); r+=1
_esp(ws,r, " TOTAL  DEL  ACTIVO",None,ta25,ta24, " TOTAL DEL PASIVO Y PATRIM. NETO",None,tp25+pn25,tp24+pn24_rx, a_bg=C_HDR,p_bg=C_HDR,a_bold=True,p_bold=True); r+=1
for ci in range(1,10): ws.cell(r-1,ci).font = Font(bold=True,name="Arial",size=9,color="FFFFFF")
...
def _nota_tbl(ws, r, titulo, filas, col_hdr="En pesos con centavos"):
    ws.merge_cells(start_row=r, start_column=1, end_row=r, end_column=2)
    c = ws.cell(r, 1); c.value = titulo; c.fill = fl(C_SEC)
    c.font = fw(bold=True, color=C_HDR, size=10)
    c.alignment = Alignment(horizontal="left", vertical="center")
    r += 1
    ws.cell(r, 1).value = "Cuenta"
    ws.cell(r, 1).fill  = fl(C_HDR); ws.cell(r, 1).font = fw(bold=True, color="FFFFFF", size=9)
    ws.cell(r, 2).value = col_hdr
    ws.cell(r, 2).fill  = fl(C_HDR); ws.cell(r, 2).font = fw(bold=True, color="FFFFFF", size=9)
    ws.cell(r, 2).alignment = Alignment(horizontal="right")
    r += 1
    for label, val in filas:
        ws.cell(r, 1).value = label; ws.cell(r, 1).font = fw(size=9)
        if val is not None:
            c2 = ws.cell(r, 2); c2.value = val
            c2.number_format = NF; c2.font = fw(size=9)
            c2.alignment = Alignment(horizontal="right")
        r += 1
    return r + 1

r = _nota_tbl(ws, r, "2.1 Caja y Bancos", [
    ("Caja", caja25_caja),
    ("Banco", caja25_banco),
    ("", caja25),
])
r = _nota_tbl(ws, r, "2.2 Creditos por Ventas", [
    ("Deudores por Ventas", cv25),
    ("", cv25),
])
r = _nota_tbl(ws, r, "2.3 Otros Creditos", [
    ("IVA Saldo Técnico", oc25_iva),
    ("Saldo Libre Disponibilidad", oc25_sld),
    ("Impuesto a los Débitos y Créditos", oc25_dbc),
    ("Retención Ganancias Sufrida", oc25_ret),
    ("", oc25),
])
r = _nota_tbl(ws, r, "2.4 Bienes de Cambio", [
    ("Bienes de Cambio", bc25),
    ("", bc25),
])
r = _nota_tbl(ws, r, "2.5.1 Comerciales", [
    ("Proveedores", dc25),
    ("", dc25),
])
r = _nota_tbl(ws, r, "2.5.2 Fiscales", [
    ("IIBB BSAS a pagar", df25_bsas),
    ("IIBB CABA a pagar", df25_caba),
    ("", df25),
])
r = _nota_tbl(ws, r, "2.5.3 Remuneraciones y Cargas sociales", [
    ("Cargas sociales a pagar", rem25),
    ("Sueldos y Jornales a pagar", None),
    ("", rem25),
])
r = _nota_tbl(ws, r, "2.5.4 Deudas Sociales", [
    (f"Cuenta particular {SOCIO}", ds25),
    ("", ds25),
])
r = _nota_tbl(ws, r, "2.6 Resultados Financieros y por Tenencia", [
    ("Recpam", recpam25_adj),
    ("", recpam25_adj),
])
r = _nota_tbl(ws, r, "2.7 Otras deudas", [
    ("", None),
])
r = _nota_tbl(ws, r, "2.8 Otros Egresos", [
    ("", None),
])
...
page_setup(wb["Caratula"])
page_setup(wb["ESP"],      landscape=True)
page_setup(wb["E.R."])
page_setup(wb["EEPN"],     landscape=True)
page_setup(wb["EF. IND."])
page_setup(wb["B. Uso"],   landscape=True)
page_setup(wb["Costo"])
page_setup(wb["Gastos"],   landscape=True)
page_setup(wb["NOTAS"])
